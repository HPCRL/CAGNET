...
def train(inputs, weight1, weight2, adj_matrix, am_partitions, optimizer, order, data, rank, size, group, row_groups, col_groups, horizontal_tiled, dry=False):
    global ht
    global run
...
def oned_partition(rank, size, inputs, adj_matrix, data, features, classes, device, mmorder):
    #global row_count
    #global col_count
    '''
...
def run(rank, size, inputs, adj_matrix, data, features, classes, device):
    ...
    for c in candidate_orders: 
        inputs_loc, adj_matrix_loc, am_pbyp, ht_tmp = oned_partition(rank, size, inputs, adj_matrix, data, 
                                                                features, classes, device, c)
        ...
        if cur_time < best_time:
            best_time =  cur_time
            best_order = c
        print(f'3-epoch time: cur_order:{c}; cur_time:{cur_time:.4f}; best_order:{best_order}; best_time:{best_time}')
        del outputs 
        del inputs_loc
        del adj_matrix_loc
        del am_pbyp
        # del horizontal_tiled
        dist.barrier(group)  
    mmorder = best_order
    print('Using best order: ', mmorder)

    inputs_loc, adj_matrix_loc, am_pbyp, horizontal_tiled = oned_partition(rank, size, inputs, adj_matrix, data,
                                                                features, classes, device, mmorder)
